...
class Finder {
  ...
  clone() {
    ...
    finder._whereAndOr = this._whereAndOr;
    finder._limit = this._limit;
    finder._order = this._order;
    finder._join = this._join;
    finder._many = this._many;
    return finder;
  }
   ...
   * @param {string} [options.as] 输出别名, 可以使用 -> 命名空间
   * @param {string} [options.columns] 需要的结果列
   * @param {boolean} [options.parallel] 并行查询结果
   * @returns {Finder}
   */
  ...
  async _fetchAfter(result) {
    ...
    for (let [model, options] of many) {
      ...
      const _fetchManyResult = async instance => {
        ...
        const manyResults = refValue !== undefined ? await finder.clone().where({ [options.fk]: refValue }).all(...columns) : [];
