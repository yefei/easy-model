...
class Finder {
  ...
  _orderBuilder(builder) {
    ...
    const joinEnable = Object.keys(this._join).length > 0;
    builder.order(...this._order.map(i => {
      if (joinEnable && !i.includes('.')) {
        if (i.startsWith('-')) {
  ...
  async _fetch(columns, isOne = false) {
    ...
    const resutl = await this._query.query(builder => {
      // 在有 join 的情况下如果没有指定目标表则默认使用当前表
      builder.select(...columns.map(i => !_notJoin && typeof i === 'string' && !i.includes('.') ? `${this._options.name}.${i}` : i));
