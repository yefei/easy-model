...
class Finder {
  ...
  _joinPrep(builder) {
    Object.values(this._join).forEach(([joinModel, options]) => {
      if (!options.on) {
        options.on = {
          [`${options.as}.${options.ref}`]:
            builder.quote(`${this._model[OPTIONS].name}.${options.fk}`),
        };
      }
      builder.join(
  ...
  async _fetch(isOne = false) {
    ...
    resutl.forEach(row => {
      ...
      if (_join.length === 0) {
        ...
        instanceMap[pkval] = this._model._createInstance(row);
        return;
      }
      // 有 join
      if (!isOne) {
        ...
        instance = instanceMap[pkval] || (instanceMap[pkval] = this._model._createInstance(thisData));
      }
      _join.forEach(([model, options]) => {
        if (row[options.as]) {
          const joinInstance = model._createInstance(row[options.as]);
          if (options.asList) {
            if (!instance[options.as]) instance[options.as] = [];
            instance[options.as].push(joinInstance);
          } else {
            instance[options.as] = joinInstance;
          }
        }
      });
    });
    return isOne ? instance : Object.values(instanceMap);
  }
