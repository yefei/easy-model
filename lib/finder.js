...
class Finder {
  ...
  join(joinModel, options) {
    if (typeof joinModel === 'string') {
      if (this._options.join && joinModel in this._options.join) {
        options = Object.assign({ as: joinModel }, this._options.join[joinModel], options);
        joinModel = options.model;
      } else {
        throw new UndefinedRelationException(`UndefinedRelationException: ${this._options.name} join ${joinModel}`);
      }
    }
  ...
  many(manyModel, options) {
    if (typeof manyModel === 'string') {
      if (this._options.many && manyModel in this._options.many) {
        ...
        throw new UndefinedRelationException(`UndefinedRelationException: ${this._options.name} many ${manyModel}`);
      }
    }
    options = Object.assign({
